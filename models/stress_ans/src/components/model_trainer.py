"""stress_ans — ModelTrainer

Source notebook: notebooks/source.ipynb
Notebook architecture: MultimodalStressNet (Conv1d ECG + FC HRV + cross-attention) + FocalLoss.

This pipeline currently trains on the synthetic 7 HRV / EDA features features produced by
DataIngestion._load_dataframe(). To upgrade to the notebook's full
architecture you'd extend `_load_dataframe` to return the raw raw ECG + HRV time-series
...
class ModelTrainer:
    ...
    def _build_model(self):
        # Multimodal NN in the notebook → small MLP on the synthetic HRV features.
