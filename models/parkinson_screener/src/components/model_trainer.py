"""parkinson_screener — ModelTrainer

Source notebook: notebooks/source.ipynb
Notebook architecture: RandomForest / XGBoost / LightGBM on VGRF + voice features (RF n=300 max_depth=10).

This pipeline currently trains on the UCI Parkinsons voice (22 numeric) features produced by
DataIngestion._load_dataframe(). To upgrade to the notebook's full
architecture you'd extend `_load_dataframe` to return the raw WearGait IMU + voice waveform
