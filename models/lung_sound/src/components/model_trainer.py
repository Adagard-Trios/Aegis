"""lung_sound — ModelTrainer

Source notebook: notebooks/source.ipynb
Notebook architecture: MultiFeatureLungCNN (Mel/MFCC/Chroma fusion, 2D Conv stack), CrossEntropyLoss weighted.

This pipeline currently trains on the ICBHI per-cycle scalar audio features (RMS, ZCR, percentiles) features produced by
DataIngestion._load_dataframe(). To upgrade to the notebook's full
architecture you'd extend `_load_dataframe` to return the raw wav audio 16 kHz raw
...
class ModelTrainer:
    ...
    def _build_model(self):
        # Notebook uses a 2D CNN on triple-feature spectrograms. Per-cycle scalar
        # features + RandomForest is the closest faithful baseline at this layer.
