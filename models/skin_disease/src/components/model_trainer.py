"""skin_disease — ModelTrainer

Source notebook: notebooks/source.ipynb
Notebook architecture: EVA02 ViT-Large (CLIP-aligned), 7-class, FocalLoss(gamma=2).

This pipeline currently trains on the HAM10000 metadata (age + dx label, image_path) features produced by
DataIngestion._load_dataframe(). To upgrade to the notebook's full
architecture you'd extend `_load_dataframe` to return the raw 224x224x3 RGB dermoscopy image
...
class ModelTrainer:
    ...
    def _build_model(self):
        # Real architecture is a Vision Transformer on dermoscopy images. With
        # only the age metadata available (image_path is dropped by the default
        # numeric preprocessor), we train a baseline tabular RF that lets the
        # rest of the pipeline land. Image trainer is a follow-up upgrade.
