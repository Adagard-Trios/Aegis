"""retinal_disease — ModelTrainer

Source notebook: notebooks/source.ipynb
Notebook architecture: EfficientNetV2-M (head 1280->512->256->8 logits), AsymmetricLoss multi-label.

This pipeline currently trains on the ODIR-5K metadata (age + label, image_path) features produced by
DataIngestion._load_dataframe(). To upgrade to the notebook's full
architecture you'd extend `_load_dataframe` to return the raw 224x224x3 RGB fundus image
